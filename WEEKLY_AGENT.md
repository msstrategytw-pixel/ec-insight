...
3. 只看**上期之後**的新回饋。將其納入本次選材判斷：
   - 標記「沒用」且有原因者 → 本期避免選同類型內容；並在本期產出後，把該案例整理成一行，寫進本期編輯後記的「回饋與調整」段落，建議顧問將其加入「評分標準」的範例庫（agent 無法寫入 Google Docs，只能建議）。
   - 標記「有用」者 → 該類型內容可適度增加權重。
4. 在 `editor_note` 中簡述：本期收到幾則回饋、有用／沒用各幾則、據此做了什麼調整。
